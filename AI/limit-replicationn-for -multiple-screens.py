...
screen_widths, screen_heights = pyautogui.size()
...
while True:
    ...
    if ball_info is not None:
        ...
        if dist < THRESHOLD_DISTANCE:
            # Map the coordinates to the screen resolution of the screen where the ball is located
            cursor_x = int((ball_x / frame.shape[1]) * screen_widths)
            cursor_y = int((ball_y / frame.shape[0]) * screen_heights)
